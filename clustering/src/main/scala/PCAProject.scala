...
object PCAProject extends App {
  ...
  val spark = SparkSession.builder().appName("PCA_Example").master("local[2]").getOrCreate()

  // Use Spark to read in the Cancer_Data file.
  val data = spark.read.option("header","true").option("inferSchema","true").format("csv").load("datasets/cancer_data")
