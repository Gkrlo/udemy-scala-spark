...
object LinearRegressionExample extends App {
  ...
  val spark = SparkSession.builder().master("local[2]").appName("ModelEv LR").getOrCreate()
  val data = spark.read.format("libsvm").load("datasets/sample_linear_regression_data.txt")
  ...
  model.transform(test).select("features","label","predictions").show()
